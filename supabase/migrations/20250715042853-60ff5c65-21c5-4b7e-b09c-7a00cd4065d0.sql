...
INSERT INTO website_content (section_key, title, subtitle, description, is_active)
VALUES 
('contact_hero', 'Contact Us', 'Get in Touch', 'Ready to transform healthcare delivery? Let us discuss how we can help extend your clinical services into the home.', true)
ON CONFLICT (section_key) DO UPDATE SET 
  title = EXCLUDED.title,
  subtitle = EXCLUDED.subtitle,
  description = EXCLUDED.description,
